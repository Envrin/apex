...
use apex\UpgradeException;
use apex\core\components;
use apex\core\io;
use apex\pkg\package;
use apex\pkg\pkg_config;
...
/**
* Handles all upgrade functions, including create upgrade point, 
* publish, download, install, and rollback.
*/
class upgrade
{
...
/**
* Create a new upgrade point.  Upon creation, all files within the 
* package will be hashed, then upon publication, all modified files will be included.
* 
...
public function create(string $pkg_alias, string $version = '')
{
    ...
    // Debug
    debug::add(3, fmsg("Starting create upgrade point for package, {1}", $pkg_alias), __FILE__, __LINE__);
    ...
    // Debug
    debug::add(4, fmsg("Successfully added new upgrade point to database for package, {1}", $pkg_alias), __FILE__, __LINE__);

    // Create file hash
    $this->create_file_hash((int) $upgrade_id);

    // Debug
    debug::add(1, fmsg("Successfully created new upgrade point for package, {1}", $pkg_alias), __FILE__, __LINE__);
...
/**
* Create a SHA1 hash of all files pertaining 
* to the package.  This is used for upgrades, as it hashes 
( all files upon creating an upgrade point, then checks against the current hashes 
* when publish the upgrade to see which files were modified.
...
public function create_file_hash(int $upgrade_id)
{
    ...
    // Debug
    debug::add(4, fmsg("Starting to create file hash for upgrade for package, {1}", $package), __FILE__, __LINE__);
    ...
    // Debug
    debug::add(3, fmsg("Successfully compiled file hash for upgrade point of package, {1}", $package), __FILE__, __LINE__);
...
/**
* Compiles an upgrade.  Goes through the SHA1 hashes created against each file when 
* the upgrade point was created, and determines which components / files have been added, updated, 
* and deleted.  Compiles them as necessary to be uploaded to the repository.
*
*     @param int $upgrade_id The ID# from the 'internal_upgrades' file which to compile
*     @return Returns the filename or the zip archive, which is stroed in the /tmp/ directory.
*/
public function compile($upgrade_id) 
{
    ...
    // Debug
    debug::add(4, fmsg("Compiling upgrade for package {1}, version {2}", $upgrade['package'], $upgrade['version']), __FILE__, __LINE__);
    ...
    // Create tmp directory
        $this->upgrade_dir = SITE_PATH . '/tmp/' . $upgrade['package'];
    ...
    // Archive file
    $archive_file = 'upgrade-' . $upgrade['package'] . '_' . str_replace(".", "_", $upgrade['version']) . ".zip";
    ...
    // Debug
    debug::add(4, fmsg("Successfully compiled upgrade, and archived it at {1}", $archive_file), __FILE__, __LINE__);
...
/**
* Compile a single component.  Will get the .php file of component, and 
* check to see whether or not it is updated or added, and process accordingly.
...
protected function compile_component(array $row)
{
    ...
    // Debug
    debug::add(5, fmsg("Compiling single component for upgrade, type: {1}, package: {2}, parent: {3}, alias: {4}", $row['type'], $row['package'], $row['parent'], $row['alias']), __FILE__, __LINE__);
...
/**
* Compile a single file.  Checks the current SHA1 hash against 
* the one stroed upon upgrade point creation, and adds file to upgrade if needed.
...
/**
* Publishes an upgrade to the appropriate 
* repository
* 
...
public function publish(int $upgrade_id)
{
    ...
    // Debug
    debug::add(2, fmsg("Starting to publish upgrade for package {1}, version {2}", $upgrade['package'], $upgrade['version']), __FILE__, __LINE__);
...
public function install(string $pkg_alias)
{
    ...
    // Debug
    debug::add(3, fmsg("Contacting repository, and requesting available upgrades for package {1}", $pkg_alias), __FILE__, __LINE__);
    ...
    foreach ($response['upgrades'] as $vars) { 
        ...
        // Save zip file
        $zip_file = SITE_PATH . '/tmp/' . $pkg_alias . '_' . str_replace('.', '_', $vars['version']) . '.zip';
    ...
    // Debug
    debug::add(1, fmsg("Successfully upgraded package {1} to version {2}", $pkg_alias, $new_version), __FILE__, __LINE__);
...
protected function install_from_zip(string $pkg_alias, string $version, string $zip_file)
{
    ...
    // Debug
    debug::add(3, fmsg("Starting to upgrade package {1} to version {2} from zip file", $pkg_alias, $version), __FILE__, __LINE__);

    // Unpack zip file
    $this->upgrade_dir = SITE_PATH . '/tmp/upgrade_' . $pkg_alias . '_' . $version;
    if (is_dir($this->upgrade_dir)) { io::remove_dir($this->upgrade_dir); }
    ...
    // Debug
    debug::add(4, fmsg("Installing upgrade, loaded upgrade class file, package {1}, version {2}", $pkg_alias, $version), __FILE__, __LINE__);
    ...
    // Debug
    debug::add(4, fmsg("Installing upgrade, copied over all files as necessary for package {1}, version {2}", $pkg_alias, $version), __FILE__, __LINE__);
    ...
    // Debug
    debug::add(4, fmsg("Installing upgrade, successfully created / deleted all necessary components for package {1}, version {2}", $pkg_alias, $version), __FILE__, __LINE__);
    ...
    // Debug
    debug::add(3, fmsg("Successfully installed single upgrade for package {1}, version {2}", $pkg_alias, $version), __FILE__, __LINE__);
...
*     @param string $verswion The version to rollback to
*/
