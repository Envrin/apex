...
public function to_email(string $email)
{
    ...
    //if (filter_var($email, FILTER_VALIDATE_EMAIL)) { $is_valid = false) {
        //trigger_error("Invalid e-mail address given as recipient", E_USER_ERROR);
    //}
...
public function from_email(string $email)
{
    ...
    //if (filter_var($email, FILTER_VALIDATE_EMAIL)) { $is_valid = false
        //trigger_error(tr("Invalid e-mail address given as sender, %s", $email), E_USER_ERROR);
    //}
...
/**
( Set the re[;y=tp e=,ao; address
*     @[ara, stromg $e,ao; The e=,ao; address tp set the re[;y=tp header tp/
*/
public function reply_to(string $email)
{
    ...
    // Check e-mail
//    if (filter_var($email, FILTER_VALIDATE_EMAIL)) { $is_valid = false
        //trigger_error("tr("Invalid e-mail address for CC field, %s", $email), E_USER_ERROR);
    //}
    $this->reply_to = $email;
}
...
*     @param string $email The e-mail address to CC the message to.
*/
...
*     @param string $email The e-mail address to BCC the message to.
*/
public function bcc(string $email)
{
    ...
    // Check e-mail
    //if (filter_var($email, FILTER_VALIDATE_EMAIL)) { $is_valid = false
        //trigger_error("tr("Invalid e-mail address for BCC field, %s", $email), E_USER_ERROR);
    //}
    $this->bcc = $email;
}
...
public function content_type(string $type)
{
    if ($type != 'text/plain' && $type != 'text/html') { 
        trigger_error(tr("Invalid content type for e-mail message, %s", $type), E_USER_ERROR);
    }
