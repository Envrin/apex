...
{ 
    ...
    // Get HTML
    $html = "<button type=\"submit\" name=\"$name\" value=\"$value\" class=\"btn btn-primary btn-lg\">$label</button>";
    return $html;
...
{ 
    ...
    if (isset($attr['data_source'])) { 
        $html .= $this->hashes->parse_data_source($attr['data_source'], $value, 'select', $package);
    } else { $html .= $text; }
    $html .= "</select>";
...
{ 
    ...
    $curdata = app::get_currency_data(app::_config('transaction:base_currency'));
...
{ 
    ...
    if (isset($attr['href']) && $attr['href'] == 'route') { 
        $nav_func = "<a href=\"/" . app::get_uri() . "?page=~page~\">";
    } else { 
...
{ 
    ...
    $comp_file = $this->theme_dir . '/components/dropdown_alert.tpl';
...
{ 
    ...
    // Get HTML of dropdown item
    $comp_file = $this->theme_dir . '/components/dropdown_message.tpl';
    if (file_exists($comp_file)) { 
        $tag_html = file_get_contents($comp_file);
