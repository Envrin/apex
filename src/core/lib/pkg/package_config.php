...
/**
* Handles package configuration files (/etc/ALIAS/package.php), 
* installation loading and installation of all components within
*/
...
/**
* Loads a package configuration file, and ensures any 
* omitted arrays within package configuration are set as blank arrays to avoid 
* errors in the rest of the class.
*/
...
/**
* Goes through the package.php configuration file, and 
* updates the database as necessary.  Ensures to update existing records 
* as necessary, and delete records that have been removed from the package.php file.
*/
...
/**
* Adds / updates the configuration variables as necessary from 
* the package.php $this->config() array.
*/
protected function install_config_vars($pkg) 
{
    ...
    foreach ($pkg->config as $alias => $value) {
        ...
        // Add to redis
        if (!registry::$redis->hexists('config', $comp_alias)) { 
            registry::$redis->hset('config', $comp_alias, $value);
        }
    }
    ...
    foreach ($chk_config as $chk) { 
        ...
        registry::$redis->hdel('config', $comp_alias);
    }
...
/**
* Adds / updates / deletes the hashes within the package.php 
* file's $this->hash array.  Hashes are used as key-value paris to easily 
* populate select / radio / checkbox lists.
*/
protected function install_hashes($pkg) 
{
    ...
    foreach ($pkg->hash as $hash_alias => $vars) { 
        ...
        registry::$redis->hset('hash', $comp_alias, json_encode($vars));
    ...
    foreach ($chk_hash as $chk) { 
        ...
        registry::$redis->hdel('hash', $this->pkg_alias . ':' . $chk);
    }
...
/**
* Adds a single menu to the 'cms_menus' table, and is used by the 
* above install_menu() function.
*
...
/**
* Gets the 'order_num' / position of a menu.  Modifies the 'order_num' column as necessary 
* to make room for the menu as well.
* 
...
/**
* Get the current menu row, where our new menu is going to 
* be placed.  Used in case if new menu is before / after a header row, to get 
* the next / last header row.
*
...
public function update_redis_menus()
{
    ...
    foreach ($areas as $area) { 
        ...
        // Update redis database
        registry::$redis->hset('cms:menus', $area, json_encode($menus));
    }

}

/**
* Installs / updates the boxlists from the package.php 
* configuration file.  These are generally used for settings 
* pages, such as user / financial settings in the admin panel.
*/
...
/**
* Re-orders the tab pages in a tab control  Used when a tab page 
* is added / removed from a tab control.
* 
