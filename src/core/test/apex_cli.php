...
public function test_create_package()
{
    ...
    $repo_dir = SITE_PATH . '/data/repo/' . $this->pkg_alias;
    if (is_dir($repo_dir)) { io::remove_dir($repo_dir); }
    DB::query("DELETE FROM repo_packages WHERE alias = %s", $this->pkg_alias);
    DB::query("DELETE FROM repo_upgrades WHERE package = %s", $this->pkg_alias);
...
public function test_scan()
{
    ...
    $value2 = registry::$redis->hget('config', $this->pkg_alias . ':name');
    ...
    registry::update_config_var($this->pkg_alias . ':name', 'update Test');
    ...
    $value2 = registry::$redis->hget('config', $this->pkg_alias . ':test_id');
    ...
    $vars = json_decode(registry::$redis->hget('hash', $this->pkg_alias . ':houses'), true);
    ...
    $name = DB::get_field("SELECT display_name FROM cms_menus WHERE area = 'admin' AND parent = 'testing' AND alias = 'create'");
    ...
    // Check package configuration
    $this->check_package_configuration($new_pkg);

}

/**
* Check package configuration.  Checks the /etc/PACKAGE/package.php config 
* file, and ensures all config vars, hashes, menus, and everything else matches up 
* with what is in the database and redis
*/
protected function check_package_configuration($pkg = '')
{
    ...
    foreach ($pkg->config as $key => $value) { 
        ...
        // Check redis
        $data = registry::$redis->hget('config', 'unit_test:' . $key);
    ...
    foreach ($pkg->hash as $hash_alias => $vars) { 
        ...
        // Check redis
        $data = registry::$redis->hget('hash', 'unit_test:' . $hash_alias);
    ...
    foreach ($pkg->menus as $vars) { 
        ...
        $this->assertequals($vars['name'], $row['display_name'], "Menu does not have correct name, alias: $vars[alias], name: $vars[name]");
        ...
        foreach ($submenus as $alias => $name) { 
            ...
            $this->assertequals($name, $row['display_name'], "Submenu does not have correct name, parent: $vars[alias], alias: $alias, name: $name");
        }
    }
...
protected function check_redis_component($type, $alias, $package, $parent)
{
    ...
    $ok = registry::$redis->sismember('config:components', $line);
    ...
    $chk = $type . ':' . $alias;
    $var = registry::$redis->hget('config:components_package', $chk);
...
protected function verify_component_created($type, $comp_alias, $owner, $files)
{
    ...
    elseif (in_array($type, array('lib', 'template', 'tabpage', 'test'))) { $ok = false; } 
...
public function provider_create()
{
    ...
    // Set array
    $vars = array(
        array('ajax', $pkg . ':utest', '', array("src/$pkg/ajax/utest.php" => "class utest extends \\\\apex\\\\ajax ")), 
        array('autosuggest', $pkg . ':find_test', '', array("src/$pkg/autosuggest/find_test.php" => "class find_test extends \\\\apex\\\\abstracts\\\\autosuggest")), 
        array('controller', $pkg . ':providers', $pkg, array()),
        array('controller', $pkg . ':providers:general', $pkg, array("src/$pkg/controller/providers/general.php" => "class general extends \\\\apex\\\\$pkg\\\\controller\\\\providers")),  
        array('cron', $pkg . ':utest', '', array("src/$pkg/cron/utest.php" => "class utest extends \\\\apex\\\\abstracts\\\\cron")), 
        array('form', $pkg . ':utest', '', array("src/$pkg/form/utest.php" => "class utest extends \\\\apex\\\\abstracts\\\\form")), 
        array('htmlfunc', $pkg . ':utest', '', array("src/$pkg/htmlfunc/utest.php" => "class utest extends \\\\apex\\\\abstracts\\\\htmlfunc", "views/htmlfunc/$pkg/utest.tpl" => "")), 
        array('lib', $pkg . ':mytest', '', array("src/$pkg/mytest.php" => "class mytest")), 
        array('modal', $pkg . ':utest', '', array("src/$pkg/modal/utest.php" => "class utest extends \\\\apex\\\\abstracts\\\\modal", "views/modal/$pkg/utest.tpl" => '')), 
        array('tabcontrol', $pkg . ':utest', '', array("src/$pkg/tabcontrol/utest.php" => "class utest extends \\\\apex\\\\abstracts\\\\tabcontrol")), 
        array('tabpage', $pkg . ':utest:profile', $pkg, array("src/$pkg/tabcontrol/utest/profile.php" => "class profile extends \\\\apex\\\\abstracts\\\\tabpage", "views/tabpage/$pkg/utest/profile.tpl" => "")), 
        array('table', $pkg . ':utest', '', array("src/$pkg/table/utest.php" => "class utest extends \\\\apex\\\\abstracts\\\\table")), 
        array('template', "admin/testing/create", $pkg, array("views/php/admin/testing/create.php" => "namespace apex", "views/tpl/admin/testing/create.tpl" => "")), 
        array('test', $pkg . ':misc', '', array("src/$pkg/test/misc.php" => "class test_misc extends \\\\apex\\\\test")), 
...
public function test_publish_verify()
{
    ...
    $verify_components = json_decode(base64_decode(''), true);
    $verify_toc = json_decode(base64_decode(''), true);
...
public function test_delete_children()
{
    ...
    // Delete
    $this->delete_component('tabpage', $this->pkg_alias . ':utest:profile', $this->pkg_alias, array("src/$this->pkg_alias/tabcontrol/utest/profile.php" => "class profile", "views/tabpage/$this->pkg_alias/utest/profile.tpl" => ""));
...
*/
protected function delete_component($type, $comp_alias, $owner, $files)
{
    ...
    $ok = registry::$redis->sismember('config:components', $chk);
...
public function test_publish_upgrade()
{
    ...
    // Check database row
    $row = DB::get_row("SELECT * FROM repo_upgrades WHERE package = %s AND version = '1.0.1'", $this->pkg_alias);
...
public function test_upgrade()
{
    ...
    // Send request
    $response = $this->send_cli('upgrade', array());
...
public function test_create_theme()
{
    ...
    // Send request
    $response = $this->send_cli('create_theme', array('utest', 'public', '2'));
    $this->assertstringcontains($response, "Successfully created new theme");

    // Check directories
    $theme_dir = SITE_PATH . '/themes/utest';
    $dirs = array(
        $theme_dir, 
...
public function test_publish_theme()
{
    ...
    $repo_file = SITE_PATH . '/data/repo_themes/utest.zip';
    if (file_exists($repo_file)) { @unlink($repo_file); }
    DB::query("DELETE FROM repo_themes WHERE alias = 'utest'");
    ...
    file_put_contents(SITE_PATH . '/themes/utest/sections/header.tpl', "The theme header");
    file_put_contents(SITE_PATH . '/themes/utest/sections/footer.tpl', "And the footer");
    ...
    // Check files
    $this->assertfileexists(SITE_PATH . '/data/repo_themes/utest.zip');

    // Check row    
    $row = DB::get_row("SELECT * FROM repo_themes WHERE alias = 'utest'");
    $this->assertnotfalse($row);
    $this->assertisarray($row);
...
public function test_delete_theme()
{
    ...
    $this->assertdirectorynotexists(SITE_PATH . '/themes/utest');
...
public function test_install_theme()
{
    ...
    // Verify
    $this->assertdirectoryexists(SITE_PATH . '/themes/utest');
    $this->assertdirectoryexists(SITE_PATH . '/themes/utest/sections');
    ...
    $this->assertfilecontains(SITE_PATH . '/themes/utest/sections/header.tpl', "The theme header");
    $this->assertfilecontains(SITE_PATH . '/themes/utest/sections/footer.tpl', "And the footer");
...
public function test_change_theme()
{
    ...
    $this->assertequals('utest', registry::config('core:theme_public'));

    // Finish up
    registry::update_config_var('core:theme_public', 'public_coco');
...
public function test_mode()
{
    ...
    // Check redis
    $this->assertequals('prod', registry::config('core:mode'), "Did not change the server mode to 'rpdo'");
    $this->assertequals(0, registry::config('core:debug_level'), "Did not update the debug level to 0");
...
public function test_debug()
{
    ...
    $this->assertequals(2, registry::config('core:debug'));
...
public function test_server_type()
{
    ...
    $this->assertequals('dbm', registry::config('core:server_type'));
...
*/
protected function send_cli(string $action, $vars = array())
{

    // Load client
    $client = new apex_cli();
    $client->process($action, $vars);

    // Return
    return registry::get_response();
